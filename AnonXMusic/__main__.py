...
async def init():
    if (
        ...
        LOGGER(__name__).error("Assistant king client variables not defined, exiting...")
        exit()
    await sudo()
    try:
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("KingXMusic").info("Stopping king x Music Bot...")
