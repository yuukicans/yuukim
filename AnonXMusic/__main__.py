...
async def init():
    if (
        ...
        LOGGER(__name__).error("Assistant Saka client variables not defined, exiting...")
        exit()
    await sudo()
    try:
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("sakaxmusic").info("Stopping saka x Music Bot...")
